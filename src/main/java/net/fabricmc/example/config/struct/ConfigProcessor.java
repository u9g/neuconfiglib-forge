            ...
            if (exposePresent && categoryPresent) {
                ...
                for (Field optionField : categoryObj.getClass().getDeclaredFields()) {
                    ...
                    if (optionPresent) {
                        ...
                        if (optionField.isAnnotationPresent(ConfigGroupMember.class)) {
                            ConfigGroupMember annotation = optionField.getAnnotation(ConfigGroupMember.class);
                            option.accordionId = annotation.groupId();
                        }
                        ...
                        if (optionType.isAssignableFrom(boolean.class) && optionField.isAnnotationPresent(ConfigGroupHeader.class)) {
                            ConfigGroupHeader configEditorAnnotation = optionField.getAnnotation(ConfigGroupHeader.class);
                            editor = new GuiOptionEditorAccordion(option, configEditorAnnotation.groupId());
                        }
                        if (optionType.isAssignableFrom(int.class)) {
                            if (optionField.isAnnotationPresent(ConfigEditorDropdown.class)) {
                                ...
                                throw new NotImplementedException("ConfigEditorStyle");
